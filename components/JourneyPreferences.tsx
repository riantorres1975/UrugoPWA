...
const maxWalkOptions = [
  { value: null, label: "Sin límite" },
  { value: 300, label: "Hasta 300 m" },
  { value: 500, label: "Hasta 500 m" },
  { value: 800, label: "Hasta 800 m" },
] as const;

const extraMinuteOptions = [
  { value: 5, label: "Hasta 5 min más" },
  { value: 10, label: "Hasta 10 min más" },
  { value: 15, label: "Hasta 15 min más" },
] as const;

const choiceClass = (selected: boolean) =>
  `min-h-11 rounded-xl border px-2 py-2 text-xs font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-lima ${selected ? "border-lima bg-lima text-ink-900" : "ov-border ov-text ov-pill"}`;
...
export default function JourneyPreferences({ value, onChange, disabled, settings, onSettingsChange }: { value: JourneyPreference; onChange: (value: JourneyPreference) => void; disabled?: boolean; settings: JourneySettings; onSettingsChange: (settings: JourneySettings) => void }) {
  ...
  const maxWalkLabelId = useId();
  const extraMinutesLabelId = useId();
  return <fieldset disabled={disabled} className="ov-border border-b px-4 py-3 disabled:opacity-50">
    ...
    <div className="mt-2 grid grid-cols-3 gap-1" role="group" aria-label="Preferencia de viaje">
      {([["nearby", "Menos caminata"], ["balanced", "Equilibrada"], ["fastest", "Más rápida"]] as const).map(([key, label]) => <button key={key} type="button" aria-pressed={key === value} onClick={() => onChange(key)} className={choiceClass(key === value)}>{label}</button>)}
    </div>
    ...
    <div id={settingsId} hidden={!expanded} className="space-y-3 pb-2">
      <div>
        <p id={maxWalkLabelId} className="ov-text text-xs font-medium">Caminata máxima en todo el viaje</p>
        <div role="radiogroup" aria-labelledby={maxWalkLabelId} className="mt-2 grid grid-cols-2 gap-2">
          {maxWalkOptions.map((option) => {
            const selected = settings.maxWalkM === option.value;
            return <button key={option.label} type="button" role="radio" aria-checked={selected} onClick={() => onSettingsChange({ ...settings, maxWalkM: option.value })} className={choiceClass(selected)}>{option.label}</button>;
          })}
        </div>
      </div>
      {value === "nearby" && <div>
        <p id={extraMinutesLabelId} className="ov-text text-xs font-medium">Tiempo extra por caminar menos</p>
        <div role="radiogroup" aria-labelledby={extraMinutesLabelId} className="mt-2 grid grid-cols-3 gap-1">
          {extraMinuteOptions.map((option) => {
            const selected = settings.extraMinutes === option.value;
            return <button key={option.value} type="button" role="radio" aria-checked={selected} onClick={() => onSettingsChange({ ...settings, extraMinutes: option.value })} className={choiceClass(selected)}>{option.label}</button>;
          })}
        </div>
      </div>}
