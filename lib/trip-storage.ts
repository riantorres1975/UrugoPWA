...
export type SavedTrip = { version: 1; savedAt: number; journey: TripJourney; tracking: TripTrackingState };
...
export function saveTrip(journey: TripJourney, tracking: TripTrackingState) {
  ...
  try { localStorage.setItem(ACTIVE_TRIP_KEY, JSON.stringify({ version: 1, savedAt: Date.now(), journey: storedJourney, tracking })); }
